...
export default class MyScene {
    ...
    createAndPositionBombas(): void{
        ...
        [1,2,3,4,5,6].forEach((p)=>{
            ...
            clonedBomba.getChildMeshes()[0].material = clonedBomba.getChildMeshes()[0].material.clone("bomba" +p);
            if(p%2 != 1){
    ...
    addAnimColor(mesh,stoppedColor, animedColor){
        const color = mesh.matchesTagsQuery("animar") ? animedColor : stoppedColor;
        var mat = <BABYLON.PBRMaterial>mesh.getChildMeshes()[0].material.clone();
    ...
    animateWTurbines():void{
        ...
        this._scene.onBeforeRenderObservable.add(() => {
            this._scene.getMeshesByTags("wturbine && animar", function (ct:Mesh) { 
    ...
    animateBombas():void{
        this._scene.onBeforeRenderObservable.add(() => {
            this._scene.getMeshesByTags("bomba && animar", function (ct:Mesh) { 
    ...
    animateTurbinas():void{
        this._scene.onBeforeRenderObservable.add(() => {
            this._scene.getMeshesByTags("turbina && animar", function (ct:Mesh) { 
                    ct.getChildMeshes()[0].rotate(BABYLON.Axis.Y, -0.03, BABYLON.Space.LOCAL);
            });
        })
    }
